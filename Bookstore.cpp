...
namespace CPSC131::BookStore
{
	...
	/// Your welcome
	BookStore::BookStore() {}
	BookStore::BookStore(::CPSC131::BookStore::BookStore& other) {}
	...
	{
		//	TODO: Your code here
		for (DoublyLinkedList::DoublyLinkedList<Book>::Iterator it = bookList.begin(); it != bookList.end(); it++) {
	...
	bool BookStore::bookExists(std::string isbn) const
	{
		...
		return (BookStore::findBook(isbn) != bookList.end());
	}
	...
	size_t BookStore::getBookStockAvailable(std::string isbn) const
	{
		...
		if (BookStore::bookExists(isbn)) {
			return BookStore::findBook(isbn).getCursor()->getElement().getStockAvailable();
		}
	...
	Book& BookStore::getBook(std::string isbn) const
	{
		//	TODO: Your code here
		if (BookStore::bookExists(isbn)) {
			return BookStore::findBook(isbn).getCursor()->getElement();
		} else {
	...
	{
		//	TODO: Your code here
		if (BookStore::bookExists(book.getIsbn())) {
	...
	void BookStore::printInventory() const
	{
		...
		for (DoublyLinkedList::DoublyLinkedList<Book>::Iterator it = bookList.begin(); it != bookList.end(); it++) {
			std::cout << '"' << it.getCursor()->getElement().getTitle() << '"' << ", by ";
			std::cout << it.getCursor()->getElement().getAuthor() << " [";
			std::cout << it.getCursor()->getElement().getIsbn() << "] (";
			std::cout << it.getCursor()->getElement().getStockAvailable() << " in stock)";
			std::cout << std::endl;
		}
	}
	...
	void BookStore::sellToCustomer(std::string isbn, size_t price_cents, size_t quantity)
	{
		//	TODO: Your code here
		if (BookStore::bookExists(isbn)) {
		BookStore::sellToCustomer(BookStore::findBook(isbn).getCursor()->getElement(), price_cents, quantity);
		}
	}
